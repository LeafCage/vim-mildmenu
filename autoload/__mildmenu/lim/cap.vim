...
function! s:_cnvvimkeycodes(str) "{{{
  try
    let ret = has_key(s:, 'disable_keynotation') ? a:str : __mildmenu#lim#keynotation#decode(a:str)
    return ret
  catch /E117:/
...
"=============================================================================
"Main:
function! __mildmenu#lim#cap#select(prompt, choices, ...) abort "{{{
...
function! __mildmenu#lim#cap#keybind(binddefs, ...) abort "{{{
  let behavior = a:0 ? a:1 : {}
  let bindacts = s:_get_bindacts(a:binddefs, function(get(behavior, 'expand') ? 's:_expand_keycodes' : 's:_noexpand'))
  return __mildmenu#lim#cap#keymappings(bindacts, behavior)
endfunction
"}}}
...
function! __mildmenu#lim#cap#keymappings(keymappings, ...) abort "{{{
  let behavior = a:0 ? a:1 : {}
